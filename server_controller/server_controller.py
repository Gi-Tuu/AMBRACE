...
DEFAULT_REFRESH_MS = 20000  # 默认界面刷新周期（毫秒）——2026-08-23 用户拍板缩短：60s→20s，状态跟上更及时
CATCHUP_MS = 2000        # 服务器操作后就绪追赶的刷新间隔（毫秒）
CATCHUP_N = 10           # 就绪追赶的最大刷新次数（检测到目标状态提前停止）
LOG_FOLLOW_MS = 3000     # 日志页实时跟随的刷新间隔（切到日志页时生效）
POLL_MS = 200
LOG_TAIL = 40
...
def _tail_log():
    """读取日志尾部（app.log + server_stderr.log 合并，应用日志在前）"""
...
def _fmt_compact(v) -> str:
    """把数值压缩成柱上标签：12000→12k、12345→12.3k、1234567→1.2M"""
    try:
        v = int(v)
    except Exception:
        return "0"
...
def _aggregate_token_trend(rows, days=7, tz_name="Asia/Shanghai", today=None) -> list:
    """把 (created_at, total_tokens) 行按「北京自然日」聚合近 N 天用量。

    纯函数（可单测）：created_at 以 UTC 落库（func.now()），这里统一按东八区自然日归桶；
    无数据日期补 0，使趋势图恒为 N 根柱子。返回 [{date, tokens}]，按日期升序。
    """
    try:
        tz = ZoneInfo(tz_name)
    except Exception:
        tz = timezone(timedelta(hours=8))  # 无 IANA tz 数据（部分精简环境）时降级为固定 UTC+8
...
def _read_token_trend(days=7, db=None, today=None) -> list:
    """只读统计：llm_usage 表近 N 天（北京自然日）Token 用量。mode=ro，表缺失/异常返回 []。"""
    if db is None:
...
class ControllerApp:
    ...
    def _build_header(self) -> None:
        ...
        tk.Button(right, text="刷新", bg=COL_CARD, fg=COL_ACCENT, activebackground=COL_CARD,
    ...
    def _build_statusbar(self) -> None:
        ...
        self.last_refresh_label = tk.Label(bar, text="上次刷新 —", fg=COL_TEXT_MUTED, bg=COL_CARD, font=(FONT, 11))
    ...
    def _make_card(self, parent, row, col, title, caption=""):
        ...
        inner.pack(fill="both", expand=True, padx=SP_LG, pady=SP_MD)
        ...
        value = tk.Label(inner, text="—", fg=COL_TEXT, bg=COL_CARD, font=(FONT, 20, "bold"))
    ...
    def _build_dashboard_page(self) -> tk.Frame:
        ...
        # 近 7 天 Token 用量趋势卡（跨 3 列，Canvas 柱状图；随 _apply_refresh 更新）
        trend_card = tk.Frame(grid, bg=COL_CARD, highlightthickness=1, highlightbackground=COL_HAIRLINE,
                              highlightcolor=COL_HAIRLINE, bd=0)
        trend_card.grid(row=2, column=0, columnspan=3, sticky="nsew", padx=SP_XS, pady=SP_XS)
        trend_inner = tk.Frame(trend_card, bg=COL_CARD)
        trend_inner.pack(fill="both", expand=True, padx=SP_LG, pady=SP_XXS)
        tk.Label(trend_inner, text="近 7 天 Token 用量", fg=COL_TEXT_SEC, bg=COL_CARD,
                 font=(FONT, 12)).pack(anchor="w")
        self.trend_canvas = tk.Canvas(trend_inner, height=60, bg=COL_CARD, highlightthickness=0, bd=0)
        self.trend_canvas.pack(fill="x", pady=(SP_XS, 0))
        self._trend = []
    ...
    def _build_log_page(self) -> tk.Frame:
        ...
        self.log_export_btn = ttk.Button(head, text="导出日志", style="Neutral.TButton", command=self.export_log)
        self.log_export_btn.pack(side="right", padx=(0, SP_XS))
        self.log_clear_btn = ttk.Button(head, text="清空显示", style="Neutral.TButton", command=self.clear_log_display)
        self.log_clear_btn.pack(side="right", padx=(0, SP_XS))
    ...
    def _sync_button_states(self) -> None:
        """按服务存活状态联动按钮可用性；busy 时全部禁用（与操作互斥，恢复后按 alive 重算）"""
        alive, ollama = self._alive, self._ollama_alive
        start_state = "disabled" if (self._busy or alive) else "normal"
        stop_state = "disabled" if (self._busy or not alive) else "normal"
        restart_state = "disabled" if self._busy else "normal"
        for b in (self.btn_start, self._dash_btn_start):
            b.config(state=start_state)
        for b in (self.btn_stop, self._dash_btn_stop):
            b.config(state=stop_state)
        for b in (self.btn_restart, self._dash_btn_restart):
            b.config(state=restart_state)
        ollama_start_state = "disabled" if (self._ollama_busy or ollama) else "normal"
        ollama_stop_state = "disabled" if (self._ollama_busy or not ollama) else "normal"
        self.btn_ollama_start.config(state=ollama_start_state)
        self.btn_ollama_stop.config(state=ollama_stop_state)
    ...
    def _do_refresh(self) -> bool:
        ...
        def _safe(fn, default):
            """单步采集失败给默认值并回显 stderr（不再吞异常导致仪表盘空白不可诊断）。"""
            try:
                return fn()
            except Exception as e:  # noqa: BLE001
                import traceback
        ...
        def job():
            # 分步降级：任何一步失败都不影响整体，队列必有结果 -> 卡片显示明确占位而非空白。
            errors = []
            try:
    ...
    # ── 服务操作后的「就绪追赶」：2s 间隔密集刷新，检测到目标状态或次数耗尽即停 ──
    ...
    def export_log(self) -> None:
        """导出当前日志：app.log + server_stderr.log 合并副本写入所选文件。"""
    ...
    def clear_log_display(self) -> None:
        """只清空 log_box 当前显示内容（不删服务器日志文件）；下次日志变化时恢复显示。"""
    ...
    def _draw_token_trend(self, trend) -> None:
        ...
        if h < 60:
            h = 190
        data = trend or []
        total = sum((t.get("tokens") or 0) for t in data)
        if not data or total <= 0:
            c.create_text(w / 2, h / 2, text="暂无数据", fill=COL_TEXT_SEC, font=(FONT, 12))
            return
        maxv = max((t.get("tokens") or 0) for t in data) or 1
        pad = 26
        top = 22
        bottom = h - 22
        plot_h = bottom - top
        n = len(data)
        slot = (w - 2 * pad) / n
        bar_w = max(6, slot * 0.55)
        for i, t in enumerate(data):
            v = t.get("tokens") or 0
            cx = pad + i * slot + slot / 2
            bar_h = (v / maxv) * (plot_h - 8)
            y0 = bottom - bar_h
            c.create_rectangle(cx - bar_w / 2, y0, cx + bar_w / 2, bottom, fill=COL_ACCENT, outline="")
            c.create_text(cx, y0 - 6, text=_fmt_compact(v), fill=COL_TEXT, font=(FONT, 9))
            c.create_text(cx, bottom + 8, text=_fmt_short_date(t.get("date", "")),
                          fill=COL_TEXT_SEC, font=(FONT, 9))
