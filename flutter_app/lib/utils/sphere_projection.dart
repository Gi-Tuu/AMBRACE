...
///
/// 2026-08-24（织网 3D 低端机体验修复）：2.5D 视图也已放开 theta（可自由翻到球背面），
/// backFar 由 0.30 下调到 0.22——整个后半球现在都可被看到，调低让背面节点更淡、翻转不刺眼、
/// 不显杂乱（正/背清晰度对比更明显，仍能看出球是立体的）。
...
/// 连线透明度参数（2026-08-24 织网 3D P2 提亮）：下限 0.06→0.14、strength 权重 0.32→0.36，
/// 让连线更清晰可读（3D 视图此前未渲染连线、2.5D 偏淡）。
const double kWeaveEdgeAlphaFloor = 0.14;
const double kWeaveEdgeAlphaStrengthWeight = 0.36;

/// 高密度判定：连线数超过此值视为高密度（此时弱边额外衰减以控噪）。
const int kWeaveEdgeHighDensityMax = 100;
/// 弱边强度阈值（strength < 此值视为弱边）。
const double kWeaveEdgeWeakStrength = 0.5;
/// 高密度下弱边的额外衰减系数。
const double kWeaveEdgeWeakFactor = 0.35;
/// 3D 连线整体提亮系数（无光遮蔽下细带易隐没，比 2.5D 更亮一档）。
const double kWeaveEdge3DBrighten = 1.15;
...
double edgeAlpha(
  ...
  double bDepthNorm,
  double strength, {
  double floor = kWeaveEdgeAlphaFloor,
  double strengthWeight = kWeaveEdgeAlphaStrengthWeight,
}) {
  final closeness = math.min(aDepthNorm, bDepthNorm);
  final base = floor + strengthWeight * strength;
  return base * closeness;
}

/// 3D 连线透明度：参照 [edgeAlpha] 语义（两端归一化深度较小值作 closeness）+ 整体提亮
/// [brighten]；高密度（连线数 > [kWeaveEdgeHighDensityMax]）时弱边额外衰减 0.35，控噪。
/// 返回 [0,1]。与 2.5D 共用同一「深度×strength」衰减思想，但整体更亮。
double weaveEdgeAlpha3D(
  double aDepthNorm,
  double bDepthNorm,
  double strength, {
  required bool highDensity,
  double brighten = kWeaveEdge3DBrighten,
}) {
  final alpha = edgeAlpha(aDepthNorm, bDepthNorm, strength) * brighten;
  final factor = (highDensity && strength < kWeaveEdgeWeakStrength)
      ? kWeaveEdgeWeakFactor
      : 1.0;
  return (alpha * factor).clamp(0.0, 1.0);
}
