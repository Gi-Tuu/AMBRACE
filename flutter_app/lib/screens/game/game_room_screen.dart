...
class _GameRoomScreenState extends State<GameRoomScreen> {
      ...
      builder: (context, _) {
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            actions: [
              // 投降：仅「在场玩家」可见，观战者/已出局者不显示
              if (p.hasSession && !isFinished && p.iAmPlayer)
                IconButton(
                  tooltip: l10n.gameSurrender,
                  icon: const Icon(Icons.flag_outlined),
                  onPressed: p.sending ? null : _surrender,
                ),
  ...
  Future<void> _surrender() async {
    ...
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(l10n.gameSurrender),
        content: Text(l10n.gameSurrenderConfirm),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(l10n.gameCancel)),
          FilledButton(onPressed: () => Navigator.pop(ctx, true), child: Text(l10n.gameSurrender)),
        ],
      ),
    );
    if (ok != true) return;
    final success = await p.surrender();
    if (!mounted) return;
    if (!success && p.error != null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(p.error!), duration: const Duration(seconds: 2)),
      );
    }
    // 成功后不 pop：单人/双人会进入结算手札视图；多人转观战，按钮随 iAmPlayer=false 自动隐藏
  }
}
