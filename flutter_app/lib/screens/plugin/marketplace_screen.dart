...
class _MarketplaceDetailScreenState extends State<MarketplaceDetailScreen> {
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          if (usage.isNotEmpty) ...[
            const SizedBox(height: 16),
            _sectionTitle(l10n.extUsageGuide),
            const SizedBox(height: 6),
            _sectionBody(usage),
          ],
          ...
          // 48a：已安装且 has_page 的页面型插件详情页可「打开」
          if (installed && item['has_page'] == true) ...[
            const SizedBox(height: 20),
            FilledButton.icon(
              onPressed: () {
                final name = item['name'] as String? ?? '';
                final page = item['page'] as String? ?? 'index.html';
  ...
  Widget _sectionBody(String t) =>
      Text(t, style: const TextStyle(fontSize: 12.5, height: 1.5, color: AppColors.textMuted));
