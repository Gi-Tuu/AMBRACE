  ...
  Future<void> _forgotPassword(BuildContext ctx) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (dialogCtx) => AlertDialog(
        title: const Text('忘记密码？修改'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: uCtrl, decoration: const InputDecoration(labelText: '用户名')),
            const SizedBox(height: 8),
            TextField(controller: pCtrl, obscureText: true, decoration: const InputDecoration(labelText: '新密码')),
            const SizedBox(height: 8),
            TextField(controller: cCtrl, obscureText: true, decoration: const InputDecoration(labelText: '确认新密码')),
          ],
        ),
        actions: [
    ...
    if (username.isEmpty || newPassword.isEmpty || newPassword != cCtrl.text) {
      if (ctx.mounted) {
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('请填写用户名与两次一致的新密码')));
      }
      return;
    }
    try {
      ...
      if (ctx.mounted) {
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('密码已重置，请用新密码登录')));
      }
    } catch (_) {
      if (ctx.mounted) {
        ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('重置失败，请检查用户名或服务器连接')));
      }
    }
  }
