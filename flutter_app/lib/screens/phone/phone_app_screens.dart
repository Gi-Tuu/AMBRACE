  ...
  Widget build(BuildContext context) {
    ...
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.photo_library_outlined, size: 56, color: IosCardColors.subtitle),
          const SizedBox(height: 12),
          Text(
            isAiTab ? l10n.noAiImages : l10n.noUploadsHint,
            style: const TextStyle(color: IosCardColors.subtitle, fontSize: 14),
          ),
        ],
      ),
    );
          ...
          for (final app in catalog)
            ListTile(
              leading: Container(
                ...
                child: Icon(_appIcon(app['key'] as String), color: Theme.of(context).colorScheme.primary),
              ),
              title: Text(app['label'] as String? ?? app['key'] as String),
              subtitle: Text(_appSubtitle(app, l10n)),
              trailing: installedKeys.contains(app['key'])
                  ? Text(l10n.installed, style: const TextStyle(color: IosCardColors.subtitle))
                  : FilledButton(
                      onPressed: () async {
    ...
    if (!ok) {
      return Center(
        child: Padding(
          ...
          child: Text(r['message'] as String? ?? l10n.searchFail,
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Row(children: [
            const Icon(Icons.travel_explore, size: 18, color: IosCardColors.subtitle),
            const SizedBox(width: 6),
            Expanded(
        ...
        children: [
          ...
          Padding(
            ...
            child: Align(
              ...
              child: Text(l10n.searchHistory, style: const TextStyle(fontSize: 12, color: Colors.grey)),
            ),
          ),
          Expanded(
            child: _searching
                ...
                : _result != null
                    ...
                    : _loading
                        ? const Center(child: CircularProgressIndicator())
                        : _history.isEmpty
                            ? Center(child: Text(l10n.searchHint, style: const TextStyle(color: Colors.grey)))
                            : ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ListTile(
                            ...
                            subtitle: Text(
                              _fmtTime(h['created_at'] as String? ?? ''),
                              style: const TextStyle(fontSize: 11),
                            ),
                            trailing: IconButton(
                              icon: const Icon(Icons.close, size: 16, color: Colors.grey),
                              onPressed: () async {
                                await ApiClient().deleteBrowserHistory(h['id'] as int);
                                _load();
                              },
                            ),
                          );
...
class _ThemeScreenState extends State<ThemeScreen> {
  static const _presets = <Map<String, Object>>[
    {'key': '', 'label': '经典·星夜', 'c1': 0xFF1C1C3A, 'c2': 0xFF6C4E7E},
    {'key': 'aurora', 'label': '极光', 'c1': 0xFF0F3443, 'c2': 0xFF34E89E},
    {'key': 'sunset', 'label': '日落', 'c1': 0xFFC33764, 'c2': 0xFF1D2671},
    {'key': 'ocean', 'label': '海洋', 'c1': 0xFF2193B0, 'c2': 0xFF6DD5ED},
    {'key': 'cherry', 'label': '樱花', 'c1': 0xFFF7B3C6, 'c2': 0xFF6A5ACD},
    {'key': 'coffee', 'label': '暖咖', 'c1': 0xFF3E2723, 'c2': 0xFFB8860B},
  ];
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(
        padding: const EdgeInsets.all(16),
        children: [
          Text(l10n.wallpaper, style: const TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          GridView.count(
            crossAxisCount: 3,
            shrinkWrap: true,
          ...
          Text(l10n.fontIconFuture, style: const TextStyle(fontWeight: FontWeight.bold)),
          const SizedBox(height: 6),
        ...
        children: [
          ...
          Expanded(
            child: _loading
                ...
                : _memos.isEmpty
                    ? Center(
                        child: Text(l10n.noMemos, style: const TextStyle(color: Colors.grey)))
                    : ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ListTile(
                            ...
                            trailing: IconButton(
                              ...
                              onPressed: () async {
                                await ApiClient().deletePhoneMemo(m['id'] as int);
                                _load();
                              },
                            ),
                          );
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(l10n.virtualPhone, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 8),
            Text(
