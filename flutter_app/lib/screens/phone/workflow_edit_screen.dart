...
class _WorkflowEditScreenState extends State<WorkflowEditScreen> {
  ...
  Future<void> _openCanvas() async {
    ...
    if (result != null && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('画布内容已同步，记得保存')),
      );
    }
  }
  ...
  Future<void> _save() async {
    ...
    if (_hasGraph) {
      ...
      if (nodes.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('画布还没有节点，请先添加步骤')),
        );
        return;
      }
    } else if (_steps.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('请填写名称并至少添加一步')),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEdit ? '编辑工作流' : '新建工作流'),
        actions: [
          IconButton(
            tooltip: '节点连线画布',
      ...
      body: Column(
        children: [
            ...
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: '名称（如：微信回消息）'),
                ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                Text(_hasGraph ? '画布节点预览' : '步骤（长按拖拽排序）',
          ...
          Expanded(
            child: _steps.isEmpty && !_hasGraph
                ? const Center(child: Text('还没有步骤，点“添加步骤”或右上角画布开始', style: TextStyle(color: Colors.grey)))
                : _hasGraph
                    ? ListView.builder(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text(stepLabelOf(action)),
                              subtitle: Text(stepSummary(s)),
                            ),
                          );
                        },
                      )
