...
class _WorkflowCanvasScreenState extends State<WorkflowCanvasScreen> {
  ...
  Future<void> _editEdge(int index) async {
    ...
    final typeLabels = {
      ...
      'always': '始终走',
      'screen_has': '屏幕有这些文字',
      'screen_empty': '屏幕没有这些文字',
    };
    final result = await showDialog<Map<String, dynamic>>(
      context: context,
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => AlertDialog(
          title: const Text('连线条件'),
          content: Column(
            ...
            children: [
              DropdownButtonFormField<String>(
                value: type,
                decoration: const InputDecoration(labelText: '何时走这条线'),
          ...
          actions: [
            TextButton(
              ...
              child: const Text('删除连线', style: TextStyle(color: Colors.redAccent)),
            ),
  ...
  Future<void> _showScreenRange() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text('屏幕范围：x 0~${w - 1}，y 0~${h - 1}'),
