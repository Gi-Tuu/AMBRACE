    ...
    try {
      ...
      setState(() {
        ...
        for (final item in catalog) {
          ...
          if (hidden) {
            _apps.add(_PhoneApp(key: key, icon: meta.icon,
      ...
      builder: (context, candidates, _) {
        ...
        return LongPressDraggable<int>(
          ...
          child: GestureDetector(
            ...
            // 点击按压 0.9 由内层 _IconPressScale 提供（Listener 实现，不与手势 Arena 冲突；
            // 编辑模式与 reduceMotion/disableAnimations 时不缩放）
            child: AnimatedScale(
              scale: hovering ? 1.05 : 1.0,
              duration: AppMotion.fast,
              curve: AppMotion.emphasized,
              child: _IconPressScale(
      ...
      bottomNavigationBar: Builder(
        builder: (context) {
          ...
          final borderColor = isDark
              ...
              : Colors.black.withValues(alpha: AppGlass.borderAlpha);
          final sigma =
              AppGlass.effectiveBlur(AppGlass.blurMedium, reduceBlur: _maybeReduceBlur(context));
...
class _IconPressScale extends StatefulWidget {
  final bool enabled;
  final Widget child;

  const _IconPressScale({super.key, required this.enabled, required this.child});

  @override
  State<_IconPressScale> createState() => _IconPressScaleState();
}

class _IconPressScaleState extends State<_IconPressScale> {
  bool _pressed = false;
