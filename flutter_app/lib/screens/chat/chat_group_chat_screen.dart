...
class _GroupMemberDialogState extends State<_GroupMemberDialog> {
  ...
  /// 成员静音/取消静音（L1 群聊群控，2026-08-25）：静音角色不参与自动选择，被 @ 仍强制回。
  Future<void> _toggleMute(Map<String, dynamic> m) async {
    final l10n = AppLocalizations.of(context)!;
    final cid = (m['id'] as num?)?.toInt() ?? 0;
    final muted = m['muted'] == true;
    try {
      await _api.setChatGroupMemberMuted(widget.groupId, cid, !muted);
      if (mounted) {
        setState(() {
          for (final x in _members) {
            if ((x['id'] as num?)?.toInt() == cid) x['muted'] = !muted;
          }
        });
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${l10n.groupMuteFail}: $e')));
      }
    }
  }
                    ...
                    for (final m in _members)
                      ListTile(
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              icon: Icon(m['muted'] == true
                                  ? Icons.volume_off
                                  : Icons.volume_up, size: 18),
                              tooltip: m['muted'] == true ? l10n.unmute : l10n.mute,
                              onPressed: () => _toggleMute(m),
                            ),
                            IconButton(
