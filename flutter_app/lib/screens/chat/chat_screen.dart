...
class _ChatScreenState extends State<ChatScreen> with RouteAware {
  ...
  /// 展开「更多功能」面板前输入框是否已聚焦（收起时仅在原已聚焦时才恢复）。
  bool _inputFocusedBeforePanel = false;
  ...
  void _toggleMorePanel() {
    ...
    if (_morePanelOpen) {
      // 展开时记录输入框当前聚焦状态，并收起键盘，避免面板被键盘顶起
      _inputFocusedBeforePanel = _inputFocusNode.hasFocus;
      FocusManager.instance.primaryFocus?.unfocus();
    } else {
      // 收起时【不主动请求输入框焦点】，避免用户只想切换功能却误触发键盘弹起；
      // 仅当展开面板前面板前输入框本已聚焦时才恢复焦点。
      if (_inputFocusedBeforePanel) {
