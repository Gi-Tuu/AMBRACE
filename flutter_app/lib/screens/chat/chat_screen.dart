...
class _ChatScreenState extends State<ChatScreen> with RouteAware {
  ...
  Future<void> _showVoiceRecorder() async {
    ...
    await showFloatingSheet(
      ...
      child: _VoiceRecordSheet(chat: chat),
    );
  }
...
class _EmojiPanelSheetState extends State<_EmojiPanelSheet> {
                ...
                child: _marketTab
                    ...
                    : _selected == -1
                        ...
                        : (_selected >= 0 && _selected < (_packs?.length ?? 0))
                            ? _buildPackBody(_packs![_selected], _selected)
                            : _buildCustomBody(),
              ),
            ],
          ],
  ...
  Widget _buildMarketBody() {
    final l10n = AppLocalizations.of(context)!;
    if (_marketLoading && _marketPacks == null) {
      return const Center(child: CircularProgressIndicator());
    }
    if (_marketError && (_marketPacks == null || _marketPacks!.isEmpty)) {
      return Center(
        child: Text(l10n.emojiMarketUnavailable, style: const TextStyle(fontSize: 13, color: Colors.grey)),
      );
    }
    final packs = _marketPacks ?? [];
    if (packs.isEmpty) {
      return Center(
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) => _buildMarketItem(packs[i], i),
    );
  }

  Widget _buildMarketItem(Map<String, dynamic> pack, int index) {
    final l10n = AppLocalizations.of(context)!;
    final installed = pack['installed'] as bool? ?? false;
    final packId = pack['id'] as String;
    final downloading = _downloading.contains(packId);
    final iconUrl = pack['icon_url'] as String? ?? '';
    final count = pack['emoji_count'] as int? ?? 0;
    return Card(
      margin: const EdgeInsets.only(bottom: 8),
      child: InkWell(
        onTap: installed ? () => _openInstalledMarketPack(packId) : null,
        borderRadius: BorderRadius.circular(12),
        child: Padding(
              ...
              if (installed)
                Column(
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: downloading ? null : () => _uninstallMarket(index),
                ...
                FilledButton(
                  onPressed: downloading ? null : () => _downloadMarket(index),
                  style: FilledButton.styleFrom(minimumSize: const Size(0, 36)),
                  child: downloading ? Text(l10n.emojiMarketDownloading) : Text(l10n.download),
                ),
            ],
          ),
        ),
  ...
  Widget _buildMarketPackBody(Map<String, dynamic> pack, int index) {
    final l10n = AppLocalizations.of(context)!;
    ...
    return GridView.builder(
      ...
      itemBuilder: (ctx, i) {
        if (i == emojis.length) {
          return InkWell(
            onTap: () => _uninstallMarketFromBody(pack),
            borderRadius: BorderRadius.circular(10),
            child: Column(
        ...
        return InkWell(
          onTap: () => _sendMarketEmoji(url, name, meaning),
          borderRadius: BorderRadius.circular(10),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
  ...
  Widget _buildPackBody(Map<String, dynamic> pack, int index) {
    ...
    if ((pack['type'] as String? ?? '') == 'market') {
      return _buildMarketPackBody(pack, index);
    }
    final downloaded = pack['downloaded'] as bool? ?? false;
    final emojis = (pack['emojis'] as List? ?? []).cast<Map<String, dynamic>>();
    if (emojis.isEmpty) {
...
class _MessageEntranceState extends State<_MessageEntrance>
  ...
  void didChangeDependencies() {
    ...
    if (MediaQuery.disableAnimationsOf(context) || _maybeReduceMotion(context)) {
      _ctrl.stop();
      _ctrl.value = 1.0;
    }
  }
...
class _PressScale extends StatefulWidget {
  ...
  const _PressScale({required this.reduceMotion, required this.child});
...
class _BackToBottomButton extends StatelessWidget {
  ...
  const _BackToBottomButton({required this.hasUnread, required this.onTap});
