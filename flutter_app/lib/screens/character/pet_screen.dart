  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        _PetActionBar(onInteract: _interact),
      ],
    );
  }
}
...
class _PetActionBar extends StatelessWidget {
  final Future<void> Function(String action) onInteract;
  const _PetActionBar({required this.onInteract});
  ...
  Widget build(BuildContext context) {
    ...
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final reduceBlur = _petMaybeReduceBlur(context);
...
bool _petMaybeReduceBlur(BuildContext context) {
  try {
...
bool _petMaybeReduceMotion(BuildContext context) {
  try {
...
class _AnimatedPetState extends State<_AnimatedPet> with TickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    final rm = MediaQuery.disableAnimationsOf(context) || _petMaybeReduceMotion(context);
    if (rm != _reduceMotion) {
