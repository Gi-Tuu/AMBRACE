...
class _DiaryScreenState extends State<DiaryScreen> {
  ...
  Widget build(BuildContext context) {
    final l10n = AppLocalizations.of(context)!;
    return Scaffold(
      appBar: AppBar(title: Text(l10n.diaryTitle(widget.characterName))),
      body: _loading ? const Center(child: CircularProgressIndicator())
          ...
          : _entries.isEmpty ? Center(child: Text(l10n.noDiary))
          : _buildDiaryTree(),
    );
  }
    ...
    return Card(margin: const EdgeInsets.only(bottom: 8), child: Column(children: [
      InkWell(onTap: () {
        ...
        child: Row(children: [
          ...
          const Spacer(), Text(l10n.diaryCount(totalDays), style: const TextStyle(fontSize: 12, color: Color(0xFF8E8E93))),
        ]),
      )),
      ...
      InkWell(onTap: () { setState(() { if (isExpanded) {
        ...
        child: Container(width: double.infinity, padding: const EdgeInsets.only(left: 24, right: 16, top: 10, bottom: 10),
          ...
          child: Row(children: [
            ...
            const Spacer(), Text(l10n.diaryCount(entries.length), style: const TextStyle(fontSize: 11, color: Color(0xFF8E8E93))),
          ]),
        ),
      ),
      if (isExpanded) ..._groupByDay(entries).entries.map((dEntry) => _buildDaySection(year, month, dEntry.key, dEntry.value)),
    ]);
  }
    ...
    return Column(children: [
      InkWell(onTap: () { setState(() { if (isExpanded) {
        ...
        child: Container(width: double.infinity, padding: const EdgeInsets.only(left: 40, right: 16, top: 10, bottom: 10),
          decoration: BoxDecoration(border: Border(bottom: BorderSide(color: Theme.of(context).dividerColor))),
          child: Row(children: [
            Icon(isExpanded ? Icons.expand_more : Icons.chevron_right, size: 14, color: const Color(0xFFC6C6C8)),
  ...
  Widget _buildDiaryEntry(DiaryEntry entry) {
    return Container(margin: const EdgeInsets.only(left: 36, right: 8, top: 4, bottom: 4),
