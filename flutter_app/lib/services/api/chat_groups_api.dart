...
extension ChatGroupApi on ApiClient {
  ...
  /// 成员静音/取消静音（muted 布尔；静音角色不参与自动选择，被 @ 仍强制回）
  Future<Map<String, dynamic>> setChatGroupMemberMuted(
      int groupId, int characterId, bool muted) async {
    final r = await dio.put('/api/v1/chat-groups/$groupId/members/$characterId',
        data: {'muted': muted});
    return r.data as Map<String, dynamic>;
  }
