...
class FcmPushService {
  ...
  Future<void> init() async {
    ...
    // 手动初始化 Firebase（不需要 google-services.json）。幂等：避免重复 initializeApp 抛 duplicate-app。
    ...
    await _registerToken(fm);
    // 先取消旧的 token 刷新订阅（防重复 init），再保存
    _tokenRefreshSub?.cancel();
    _tokenRefreshSub = fm.onTokenRefresh.listen(_registerTokenWithBackend);
  ...
  Future<void> _setupForegroundHandler() async {
    ...
    // 幂等：仅首次创建插件/渠道并 initialize，重复 init 不重复监听。
    if (_fln == null) {
    ...
    // 前台 onMessage 订阅：先取消旧的（防重复 init 叠加），再保存
  ...
  Future<void> unregister() async {
    ...
    _initialized = false; // 保留 _deviceId，同设备复用
  }

  /// 处理通知点击深链
  static int _deepLinkRetries = 0;
  static String? _deepLinkInProgress;
  ...
  static void handleDeepLink(Map<String, dynamic> data) {
    ...
    if (ctx == null) {
      final key = 'chat:$characterId';
      // 已有进行中的重试链且 key 不同：放弃本次，防止多条 Timer 叠加
      if (_deepLinkInProgress != null && _deepLinkInProgress != key) {
        debugPrint('[FCM] deep link retry in progress for $_deepLinkInProgress, skip $key');
        return;
      }
      _deepLinkInProgress ??= key;
      ...
      if (_deepLinkRetries < 10) {
        ...
        // 耗尽后重置计数器，避免永久锁死后续通知
        _deepLinkRetries = 0;
        _deepLinkInProgress = null;
        debugPrint('[FCM] deep link retries exhausted, reset for next notification');
      }
      return;
    }
    _deepLinkRetries = 0;
  ...
  static Future<void> _openChat(int characterId) async {
    ...
    try {
      ...
      if (char == null) {
        ...
        if (ctx.mounted) {
          ScaffoldMessenger.of(ctx).showSnackBar(
            SnackBar(content: Text(l10n.charNotFoundOrDeleted), duration: const Duration(seconds: 2)),
          );
        }
        return;
      }
      ...
      // 网络错误等：提示用户而不是静默无反应
      debugPrint('[FCM] open chat failed: $e');
      if (ctx.mounted) {
        ScaffoldMessenger.of(ctx).showSnackBar(
          SnackBar(content: Text(l10n.connFailedRetry), duration: const Duration(seconds: 2)),
        );
      }
    }
  }
