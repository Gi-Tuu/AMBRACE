...
import 'moment_compose_screen.dart';
...
class _MomentsScreenState extends State<MomentsScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      if (_archiveMode) {
        ...
        if (mounted) setState(() { _archiveDays = days; _expandedDays = expanded; _loading = false; _loadError = false; });
      } else {
        ...
        if (mounted) setState(() { _moments = moments; _loading = false; _loadError = false; });
      }
    } catch (_) {
  ...
  void _startReply(int momentId, {int? commentId}) {
    setState(() { _replyingToMoment = momentId; _replyingToComment = commentId; });
  }
  ...
  /// 发布用户动态；成功返回 true（供发布面板关闭用），失败/空内容返回 false。
  Future<bool> _publishUserMoment() async {
    final l10n = AppLocalizations.of(context)!;
    var text = _publishCtrl.text.trim();
    if (text.isEmpty && _pendingImage == null) return false;
    if (_uploadingMoment) return false;
  ...
  /// 发布动态：进入独立页（微信式，自带键盘避让）
  Future<void> _showPublishSheet() async {
    await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => MomentComposeScreen(
          onSubmit: (text, image) async {
            // 复用既有发布管线：写入它依赖的控制器/待发图，再调原方法
            _publishCtrl.text = text;
            _pendingImage = image;
            final ok = await _publishUserMoment();
            _publishCtrl.clear();
            _pendingImage = null;
            return ok;
          },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        // Aurora 玻璃顶栏：半透明背景 + 0.5px 描边（本页无底栏，不加模糊保持配额余量）
  ...
  /// 空态/错误态（EmptyState 统一渲染；包在可下拉刷新的 ListView 内）
  Widget _buildEmptyOrError(AppLocalizations l10n) {
    return ListView(
      ...
      children: [
        ...
        EmptyState(
          ...
          title: _loadError ? l10n.loadFailedCheckServer : l10n.noMoments,
          action: _loadError
              ? TextButton.icon(
                  onPressed: _load,
  ...
  Widget _buildFlatView() {
    final l10n = AppLocalizations.of(context)!;
    if (_moments.isEmpty) {
      return _buildEmptyOrError(l10n);
    }
