...
class MessageBubble extends StatelessWidget {
  ...
  Widget _buildFileCard(BuildContext context) {
    ...
    final fname = meta['name'] as String? ?? '文件';
      ...
      onTap: expired
          ? () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('文件已过期（保留 5 天后自动清理）')),
              );
            }
          : () => _openFile(context),
          ...
          children: [
            ...
            Flexible(
              child: Column(
                ...
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text(fname, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600), maxLines: 2, overflow: TextOverflow.ellipsis),
                  if (fsize.isNotEmpty)
                    Text(expired ? '$fsize · 已过期' : fsize, style: TextStyle(fontSize: 11, color: expired ? Colors.red.shade300 : Colors.grey.shade500)),
                ],
              ),
            ),
          ],
      ...
      child: Column(
        ...
        children: [
              ...
              child: ConstrainedBox(
                ...
                child: Image.network(
                  ...
                  errorBuilder: (ctx, e, st) => Container(
                    ...
                    height: 140,
                    color: Colors.black.withValues(alpha: 0.05),
                    child: const Center(child: Text('图片加载失败', style: TextStyle(fontSize: 12, color: Colors.grey))),
                  ),
                ),
              ),
            ),
          ...
          if (stage.text.isNotEmpty)
            Text(stage.text, style: const TextStyle(fontSize: 15)),
  ...
  Widget _buildQuoteBlock(BuildContext context) {
    ...
    final label = q['sender'] == 'user' ? '我' : 'TA';
    return Container(
      ...
      child: Text(
        quoteDeleted ? '原消息已删除' : '$label：$content',
        maxLines: 2,
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Row(
        ...
        children: [
          _playing
              ? const Icon(Icons.stop_circle_outlined, size: 18, color: Colors.redAccent)
              : Icon(_failed ? Icons.error_outline : Icons.play_circle_outline, size: 18, color: _failed ? Colors.orange : fg),
          const SizedBox(width: 6),
          Text(
            _failed ? '播放失败' : widget.label,
            style: TextStyle(fontSize: 13, color: _failed ? Colors.orange : fg),
          ),
          if (widget.duration > 0) ...[
            const SizedBox(width: 4),
            Text('${widget.duration}"', style: TextStyle(fontSize: 12, color: fgDim)),
          ],
          if (_loading) ...[
            const SizedBox(width: 6),
