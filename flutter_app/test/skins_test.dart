...
void main() {
  ...
  group('AppTheme 皮肤构建', () {
    ...
    test('glass 皮肤 supportsDarkMode=true 且 buildSkinColors 含玻璃/极光色', () {
      final skin = SkinRegistry.get('glass');
      expect(skin.supportsDarkMode, isTrue);
      final colors = skin.buildSkinColors(brightness: Brightness.light, seedColor: Colors.blue);
      expect(colors.glassBackground, isNotNull);
      expect(colors.auroraColor1, isNotNull);
      expect(colors.auroraColor2, isNotNull);
    });

    test('AppTheme.build glass 浅/深色正常构建，scaffold 透明', () {
      final light = AppTheme.build(brightness: Brightness.light, seedIndex: 0, skinId: 'glass');
      final dark = AppTheme.build(brightness: Brightness.dark, seedIndex: 0, skinId: 'glass');
