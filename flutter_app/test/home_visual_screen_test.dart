...
import 'package:ai_companion/features/life/home_visual_screen.dart';
...
void main() {
  ...
  testWidgets('Aurora P4 冒烟：mock state → 页面加载、AppBar 标题、状态栏渲染不崩', (tester) async {
    SharedPreferences.setMockInitialValues({});
    final api = FakeApiAdapter();
    ApiClient().dio.httpClientAdapter = api;
    api.json('GET', '/api/v1/life-home/state', {
      'character_id': 1,
      ...
      'rooms': [
        {
          ...
          'name': 'Living',
          'furniture': [
            {'key': 'sofa', 'name': 'Sofa', 'gx': 2, 'gy': 6, 'gw': 3, 'gh': 1, 'rotation': 0, 'action': 'rest'},
          ],
        },
      ],
      'player': {'stamina': 80, 'mood': 60, 'hunger': 70},
      'ai': {'name': 'Alpha', 'current_status': 'reading'},
      'pets': [],
      'world': null,
      'user': null,
    });
    ...
    await tester.pump(const Duration(seconds: 4)); // 拖动提示 3s 定时器走完

    // AppBar 标题 = character_name（user 为空时回退）
    expect(find.text('Alpha'), findsOneWidget);
    // 状态栏 AI 状态行渲染
    expect(find.textContaining('reading'), findsOneWidget);
    // 无错误态
    expect(find.textContaining('加载小家失败'), findsNothing);
  });

  // Aurora P4 说明：家具 FloatingSheet 与波纹的端到端点击依赖画布 fit 坐标换算
  //（视图缩放/平移 + Expanded 区域偏移），测试成本高；FloatingSheet 组件本身已在
  // P1 朋友圈发布面板验证，波纹为页内私有组件（key=furnitureRipple），此处不重复覆盖。
}
