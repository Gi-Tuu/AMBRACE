...
import 'package:ai_companion/features/weave/weave_canvas_screen.dart';
...
void main() {
  ...
  setUp(() {
    ...
    // 织网 3D 已默认开；本组用例测 2.5D 画布（降级实现），故强制关掉 3D 取 2.5D。
    FeatureFlagService.instance.debugSetLocal('weave_3d', false);
  });

  tearDown(() {
    ApiClient().dio.httpClientAdapter = origAdapter;
    // 恢复默认：织网 3D 默认开（避免影响其它用例/后续默认）。
    FeatureFlagService.instance.debugSetLocal('weave_3d', true);
  });
