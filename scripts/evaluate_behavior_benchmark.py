...
async def main(days: int) -> None:
    ...
    # Agent：工具执行成功率（统一 status 口径，2026-08-23：blocked=拦截未执行，不计入成功率分母）
    from app.agent.status import classify as _classify
    ok = sum(1 for l in logs if _classify(l.status) == "success")
    failed = sum(1 for l in logs if _classify(l.status) == "failed")
    partial = sum(1 for l in logs if _classify(l.status) == "partial")
    blocked = sum(1 for l in logs if _classify(l.status) == "blocked")
    # 部分失败不按整体失败计（partial 计入完成，不计失败）
    done = ok + partial
    ...
    task_ok = sum(1 for t in tasks if _classify(t.status) == "success")
    ...
    print()
    print("## Agent（9.4）")
    print(f"- 工具执行总次数：{attempts}，成功（ok+partial）：{done}，失败（failed）：{failed}，成功率：{done / attempts * 100:.1f}%" if attempts else "- 工具执行总次数：0")
    print(f"- 被拦截/未执行（blocked）：{blocked}（拦截不计入成功率，非失败）")
