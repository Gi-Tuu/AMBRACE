...
echo "[1/4] 创建虚拟环境..."
[ -d "$ROOT_DIR/backend/.venv" ] || python3 -m venv "$ROOT_DIR/backend/.venv"
...
echo "[4/4] 自检..."
if ! "$ROOT_DIR/backend/.venv/bin/python" -c "import tkinter" >/dev/null 2>&1; then
    echo "  [提示] 桌面控制台需要 Tk 支持（Ubuntu/Debian：sudo apt install python3-tk；macOS 用 python.org 安装包自带）"
fi
if [ ! -d "$ROOT_DIR/backend/models/bge-m3" ]; then
...
echo ""
echo "部署完成！下一步："
echo "  1. 启动服务（Linux/macOS，前台）："
echo "       cd backend && .venv/bin/python -m uvicorn app.main:app --host 0.0.0.0 --port 8000"
echo "     （推荐 systemd 守护：sudo cp scripts/ambrace.service /etc/systemd/system/ 并替换路径后 enable --now）"
echo "     （Windows 用：backend/.venv/Scripts/python scripts/server_manager.py start）"
echo "  2. 查看连接信息："$ROOT_DIR/backend/.venv/bin/python" scripts/get_server_info.py"
