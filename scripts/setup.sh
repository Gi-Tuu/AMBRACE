...
python3 -c "import sys; sys.exit(0 if sys.version_info >= (3,10) else 1)" || { echo "[错误] 需要 Python 3.10 及以上"; exit 1; }
...
if [ ! -d backend/models/bge-small-zh-v1.5 ]; then
    echo "  [警告] 缺少向量模型目录 backend/models/bge-small-zh-v1.5，记忆系统将无法启动"
else
    echo "  [OK] 向量模型目录存在"
fi
