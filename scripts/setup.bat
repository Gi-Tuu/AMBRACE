...
"%~dp0..\backend\.venv\Scripts\python.exe" -m pip install --upgrade pip >nul
"%~dp0..\backend\.venv\Scripts\pip.exe" install -r "%~dp0..\backend\requirements.txt"
if errorlevel 1 (
...
echo [4/4] Self-check...
if not exist "%~dp0..\backend\models\bge-m3" (
...
if not exist "%~dp0..\backend\data\sqlite" (
