...
- **聊天**：流式回复、连续发送、图片理解（默认本地 OCR 识文，可选云端视觉/本地 VLM，图片不进聊天 LLM）
- **记忆系统**：向量记忆 + 每日摘要 + 置顶内容条 + 半重复融合清理
...
## 技术栈
- 后端：FastAPI + SQLAlchemy(async) + SQLite + ChromaDB（向量记忆）
- 前端：Flutter（**仅支持 Android 构建**，手机感知依赖 Android 原生服务），Provider 状态管理，Dio + WebSocket
...
scripts/    运维脚本（server_manager.py 启停/体检/修复，watchdog.py 守护，backup.py 备份，setup.bat/setup.sh 一键部署，uninstall.bat/uninstall.sh 卸载，build_apk.bat/build_apk.sh 一键打包 APK，get_server_info.py 获取服务器地址，init_db.py 手动重建表，setup_local_vlm.py 可选下载本地识图模型）
server_controller/  桌面控制台（跨平台 tkinter：Windows 双击 start_controller.vbs / Linux·macOS 执行 bash start_controller.sh）
docs/       开发文档（架构 / 规划 / 全景，仅本地仓库保留，不随开源包分发）
```
...
> ⚠️ **模型说明**：发布包内 `backend/models/` 约 800MB，含向量模型 `bge-small-zh-v1.5`（记忆系统必需）与语音转写模型 `whisper-small`（可选功能，缺失时语音转写自动降级）。请使用 Release 完整包部署，或自行放置模型目录。
...
Linux / macOS 启动（server_manager / watchdog 为 Windows 专属；桌面控制台为跨平台程序，请用下方「1.4 Linux/macOS 部署与守护」）：
```bash
cd backend
...
### 1.1.5 Windows 防火墙（可选）
局域网/手机访问被拦时，以管理员运行 `scripts\open_firewall.bat` 放行 8000 端口（Linux 防火墙见 1.4 第 5 步）。
...
### 1.3 Tailscale 远程连接（跨网络访问电脑上的服务器）
...
**7. 局域网/Tailscale 连接**
与 Windows 一致：运行 `backend/.venv/bin/python scripts/get_server_info.py` 获取地址；跨网络用 Tailscale（见 1.3 节，电脑端安装步骤同样适用于 Linux/macOS）。
...
- **后台稳定**：将本应用加入"电池优化白名单"并允许自启动/后台活动，否则息屏后轮询与通知可能被系统杀掉（厂商 ROM 差异较大）
...
- 需要「电池优化白名单」与「允许后台启动」以保证轮询与弹窗稳定

### 4. 从 GitHub 源码部署（不使用 Release 完整包）
源码仓库与发布包有两点差异，补齐后与发布包完全一致：

1. **向量模型（必需）**：`backend/models/bge-small-zh-v1.5`（约 330MB）未纳入 git。请从本仓库 **Release 附件**下载 `ai_companion_public.zip`，解压后把 `backend/models/` 整个目录复制到项目根（或直接解压整个包使用）。缺少该目录时后端会因记忆系统无法启动而报错（`setup.bat/sh` 自检也会提示）。
...
- 数据库结构变更由启动时自动迁移（幂等补列/建表），无需手动执行迁移命令
