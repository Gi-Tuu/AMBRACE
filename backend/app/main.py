...
async def lifespan(app: FastAPI):
    ...
    # 数据库版本链对齐（Alembic）：存量库 stamp head；落后于 head 时 upgrade；已在 head 则 no-op。
    # 防御性：Alembic 未就绪或执行失败只告警、不阻断启动，保证 init_db 幂等路径始终可用。
    ...
    # 注册内置工具（AMBRACE 重构步骤 8）：把 4 个内置工具（search/image_gen/note_calendar/note_memo）
    # 的执行入口登记到 ToolRegistry（幂等可重入；失败只告警不阻断启动）。
    try:
