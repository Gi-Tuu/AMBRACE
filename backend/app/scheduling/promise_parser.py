...
# 第 4 位 user_only（F1a 2026-09-08）：True=仅对 sender="user" 生效。开会/吃饭/洗澡三条纯动作
# ready 原是陪伴主动线（用户说自己离开 → 到点 AI 主动问）；AI 自己说"去吃饭了"是自述离开，
# 到点不该由 AI 反过来喊用户（语义倒置），sender="ai" 时跳过这三条。
_VAGUE_PATTERNS = [
    # "等下饭好了叫你 / 等会弄完找你 / 一会儿好了喊我" → 10 分钟
    (r"(?:等下|等会|等会儿|等一会|等一阵|一会儿|一会|稍等|待会|待会儿)\s*(?:.*?)?\s*(?:我)?(?:叫你|来叫你|叫你起来|叫你起床|喊你|喊你起来|叫我|来叫我|喊我|告诉我|找你|来找你|来找我)", 10, "ready", False),
    (r"(?:先|再|去)?\s*(?:忙|弄|干|搞|处理|写|做)\s*(?:会|一下|点)?\s*(?:事|活|东西|文件)?\s*(?:，|,)?\s*(?:完了|弄完|忙完|搞定|之后|等下|等会)\s*(?:再|就)?\s*(?:我)?(?:叫你|来叫你|喊你|叫我|来叫我|喊我|找你|来找你|来找我)", 10, "ready", False),
    # "睡醒了喊你 / 眯一会儿叫你 / 补个觉好了叫你" → 30 分钟
    (r"(?:睡醒|睡一觉|眯一会|眯会儿|补个觉|休息一会|歇会儿)\s*(?:.*?)?\s*(?:我)?(?:叫你|来叫你|喊你|叫我|来叫我|喊我|来找你)", 30, "ready", False),
    # "看完这集叫你 / 弄完手上的事找你 / 忙完来找你" → 30 分钟
    (r"(?:看完|弄完|做完|忙完|搞定|处理好|收个尾)\s*(?:这集|这部|这个|这些|手上|手头|手里|那点|剩下)?\s*(?:事|活|东西)?\s*(?:我)?(?:叫你|来叫你|叫你起来|喊你|叫我|来叫我|喊我|找你|来找你|来找我)", 30, "ready", False),
    # 陪伴主动线（2026-08-30）：无数字日常句式兜底（去开会/去吃饭/等下试/去洗澡），到点 AI 主动关心
    (r"开会去了|去开会|去忙了|去忙会儿", 60, "ready", True),
    (r"吃饭去了|去吃饭了|先去吃饭|去吃饭", 40, "ready", True),
    (r"(?:等下|等会|等会儿|一会|一会儿|待会|待会儿)\s*试(?:试)?", 10, "ready", False),
    (r"洗澡去了|去洗个澡|洗个澡|去洗澡", 30, "ready", True),
]
...
def extract_timer(
    ...
    if minutes is None:
        for pattern, default_minutes, etype, user_only in _VAGUE_PATTERNS:
            if user_only and sender != "user":
                continue  # F1a（2026-09-08）：纯动作 ready 仅对用户消息生效，AI 自述离开不建事件
            m = re.search(pattern, response)
            if not m:
                continue
...
def extract_ai_timer(
    ...
    *,
    ...
    """AI 侧承诺提取入口（F1b 2026-09-08）：source_message_id 只允许绑 AI 自身消息 id
    （文本生成时 AI 消息尚未落库则传 None），绝不指向用户消息——防止定时事件归属错乱
    （曾致 #33 事件 source 绑成用户消息）。同时受 F1a 约束：AI 自述"去吃饭了"等
    纯动作句不再生成到点 ready 事件。"""
    return extract_timer(
        response, user_id=user_id, character_id=character_id,
        session_id=session_id, source_message_id=ai_message_id, sender="ai",
    )
