    ...
    try:
        async with async_session_factory() as db:
            ...
            if not enabled:
                lg.recovered = True
                await db.commit()
                from app.scheduling.storyline_engine import close_storyline_episode
                await close_storyline_episode(character_id, "cold_war")
            ...
            if _now_naive() - created > timedelta(minutes=max_min):
                lg.recovered = True
                await db.commit()
                from app.scheduling.storyline_engine import mark_cold_war_breakthrough
...
async def _try_publish_moment(character_id: int, state_lines: str, rule: Rule) -> bool:
    ...
    try:
        ...
        spawn_background(generate_comments_for_moment(result["id"]), name="moment-comments")
    except Exception as e:
...
async def _execute_rule_behavior(
    ...
    if rule.moment:
        if await _try_publish_moment(character_id, state_lines, rule):
            ...
            spawn_background(_post_trigger_notes(character_id, user_id, rule, output_text), name="trigger-notes")
    ...
    # P0-1（2026-08-24）：注入最近聊天语境（get_last_messages 5 条摘要）与进行中话题（build_active_channel_persona），
    # 让状态触发消息承接最近聊的或与当前状态自然衔接，避免突兀换话题；失败静默
    _session_id = None
    recent_ctx = ""
    try:
        from app.application.chat_service import get_latest_session_id as _get_sid
        from app.scheduling.triggers import get_last_messages as _get_last
        _session_id = await _get_sid(user_id, character_id)
        if _session_id:
            recent_ctx = (await _get_last(_session_id, limit=5)) or ""
    except Exception:
        _session_id = None
        recent_ctx = ""
    active_persona = ""
    try:
        ...
        active_persona = await build_active_channel_persona(character_id, user_id)
    except Exception:
        active_persona = ""
    ...
    # P0-1（2026-08-24）：复用顶部取得的会话 id（无则回退重新查询），避免重复 DB 查询
    session_id = _session_id if _session_id is not None else await get_latest_session_id(user_id, character_id)
    ...
    spawn_background(_post_trigger_notes(character_id, user_id, rule, content), name="trigger-notes")
    await _after_trigger_create_preoccupation(character_id, user_id, rule)
...
async def check_state_triggers(
    ...
    try:
        async with async_session_factory() as db:
            ...
            from app.scheduling.storyline_engine import check_storyline_aftermath
            await check_storyline_aftermath(character_id, user_id)
            # 非冷战剧情线推进（v5-B）：吃醋/疲惫剧情激活时推进节点
            from app.scheduling.storyline_engine import advance_active_storylines
            ...
            for lg in logs:
                if not lg.recovered:
                    ...
                    if rule is None or not _rule_hit(rule, st):
                        lg.recovered = True
                        dirty = True
                        recovered_keys.append(lg.trigger_key)
            if dirty:
                await db.commit()
                # 自然回落结束剧情：收尾对应剧情线残留 active（冷战不走 mark_*，必须在此补）
                from app.scheduling.storyline_engine import close_storyline_episode, TRIGGER_TO_STORYLINE
                for _tk in recovered_keys:
                    _sk = TRIGGER_TO_STORYLINE.get(_tk)
                    if _sk:
                        await close_storyline_episode(character_id, _sk)
            ...
            if cw_log is not None and not cw_log.recovered:
                from app.scheduling.storyline_engine import advance_cold_war_storyline
        ...
        from app.scheduling.storyline_engine import ensure_storyline_after_trigger
