...
async def build_context_legacy(state: dict, *, stream: bool | None = None, _section_values: dict | None = None) -> dict:
    ...
    `_section_values`（试水内部）：由 context.build_context 走注册表算出的 MCP/记忆分区值
    （keys: mcp_tools/mcp_resources/memories/core_memories/anchors/open_loops）。提供时
    跳过对应内联计算（记忆轮次 +1 已在入口由注册表路径执行），其余逻辑与原实现完全一致；
    缺省（None）时按原逻辑执行——Feature Flag 关闭时的回退路径，零行为变化。
    """
