...
AGENT_FLAGS = {
    ...
    "agent_social_light_context": True,  # F1/F2（2026-08-18 用户拍板全量开启）：群聊/抖音社交短回复走轻量上下文（单次 prompt ≈-64%；回退改 False 重启即恢复全量 build_context）。与 agent_loop_group_chat/agent_loop_douyin 正交：前者管走不走 Runtime，后者管 Runtime 内是否用轻量上下文
}
