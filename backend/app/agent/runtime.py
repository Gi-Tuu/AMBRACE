...
async def _run_tool_stage(state: dict, steps: list[dict], *, character_id: int, user_id: int | None, session_id: int | None) -> bool:
    ...
    - 本地小手机工具（日历/备忘）：经 tool_runner.execute_tool 统一执行（执行入口由内置工具
      提供——权限/生命周期钩子/异常隔离），observation 注入 → 返回 True；
    ...
    for act in parsed:
        ...
        # MCP 工具（action_type=完整 mcp.{server}.{tool}）按名查；其余按 action_type 反查。
        spec = get_tool(action_type) if action_type.startswith("mcp.") else get_tool_by_action(action_type)
        payload = dict(act.payload or {})
        # MCP 工具参数需严格匹配其 inputSchema，不注入 character_id（避免多传参数被 Server 拒绝）
        if not action_type.startswith("mcp."):
        ...
        if spec is not None and spec.name in ("note_calendar", "note_memo"):
            try:
                ...
                steps.append({"action": action_type, "ok": False, "reason": "exception"})
                continue
        ...
        if spec is None or (spec.plugin and spec.plugin_action) or (spec.execute is None and not spec.plugin_action):
            steps.append({"action": action_type, "ok": False, "reason": "no executor"})
            continue
        try:
            from app.agent.tool_runner import execute_tool
            res = await asyncio.wait_for(
            ...
            _logger.warning("Runtime tool %s failed: %s", action_type, e)
            steps.append({"action": action_type, "ok": False, "reason": "exception"})
            continue
        ok = bool(res.get("status") == "ok" and (res.get("result") or {}).get("ok", True) is not False)
        steps.append({"action": action_type, "ok": ok})
...
async def build_light_social_context(state: dict) -> dict:
    ...
    # 2. 身份画像 + 关系温度（复用 context 注册表 persona 块：assemble_persona_context 同源、
    #    ctx 缓存；platform 默认 app 不裁剪为 public——群聊是家庭私群）。轻量上下文只保留
    #    persona/关系/核心/锚点/时间/语言/短回复约束，过滤完整世界认知等重分区。
    try:
        from app.agent.context.section_persona import _persona as _registry_persona
        p = await _registry_persona(state, {})
        _p_rows = []
