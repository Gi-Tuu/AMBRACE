...
async def core_memories_section(state: dict, ctx: dict) -> str:
    ...
    core, _, _ = await _inject_core_anchors_loops(state.get("character_id"), state.get("user_id", 1), ctx["trim"])
    return core


async def anchors_section(state: dict, ctx: dict) -> str:
    """anchors 分区：关系锚点（template 槽；长期画像不参与 N 轮去重）。"""
    _, anchors, _ = await _inject_core_anchors_loops(state.get("character_id"), state.get("user_id", 1), ctx["trim"])
    return anchors


async def open_loops_section(state: dict, ctx: dict) -> str:
    """open_loops 分区：开放循环（template 槽）。"""
    _, _, loops = await _inject_core_anchors_loops(state.get("character_id"), state.get("user_id", 1), ctx["trim"])
    return loops
