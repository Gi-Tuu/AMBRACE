...
async def deduplicate_memories(character_id: int, threshold: float = VECTOR_DEDUP_THRESHOLD) -> int:
    ...
    if len(vec_ids) >= 2:
        ...
        for a, b, _sim in pairs:
            ...
            # P0 护栏：锁定记忆契约「不删除」，优先级最高（locked > pinned > 重要度）
            if (ma.is_pinned and mb.is_pinned) or (ma.is_locked and mb.is_locked):
                continue
            if ma.is_locked:
                lower = b
            elif mb.is_locked:
    ...
    # ---- 执行删除（软删除 + 清向量）----
    # P0 双保险：锁定记忆契约「不删除」——即使上方判定误入 to_delete，这里统一过滤
    to_delete = {mid for mid in to_delete if not by_id[mid].is_locked}
    deleted = 0
