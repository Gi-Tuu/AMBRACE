...
from app.memory.user_facts import MUTABLE_SLOTS, get_active_user_facts, user_fact_slot_enabled_for
...
async def align_character_to_user_facts(character_id: int, user_id: int) -> dict:
    ...
    try:
        for f in await get_active_user_facts(user_id):
            ...
            # A5（2026-09-19）：门控按 user_id 解析（该账号覆盖优先），不用全局值冒充 per-user。
            if not await user_fact_slot_enabled_for(f.slot, user_id):
                continue
