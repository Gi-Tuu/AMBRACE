...
MEANING_FRESH_WINDOW_HOURS = 24           # M-P1-1：写入时达标的里程碑记忆候选窗口（小时），防衰减挤出
...
async def run_meaning_extraction(character_id: int, user_id: int) -> int:
    ...
    async with async_session_factory() as db:
        ...
        # M-P1-1（2026-08-18）：事件类 S=3 天，6h 衰减后 importance 可能跌破 60；
        ...
        items = [
            m for m in rows
            if _is_milestone(m.memory_type, m.sub_type, float(m.importance or 0))
            or (m.created_at is not None and m.created_at >= fresh_cutoff)
        ]
        if not items:
            return 0
