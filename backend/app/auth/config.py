...
def create_token(user_id: int) -> str:
    ...
    expire = datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(minutes=auth_settings.access_token_expire_minutes)
    return jwt.encode(
        {"user_id": user_id, "exp": expire, "iat": datetime.now(timezone.utc).replace(tzinfo=None)},
