"""家庭群聊应用服务（F5-a，2026-08-31 自 api/chat_groups.py 迁入）。

业务体：群 CRUD/成员管理与静音/@提及列表/消息列表/发言回应生成（旧单次 JSON 链路 +
统一 Runtime 链路）//play 群游戏命令/群记忆按发言者沉淀/群聊 trace。api/chat_groups.py
只留 FastAPI 壳（收参→调本模块→返回）并对历史名字做门面重导出（F8 删旧时移除）。

不变量：回应者三层漏斗（@必回/talkativeness 概率激活/随机兜底）、静音语义（被 @ 仍回）、
知识边界 prompt、记忆按发言者拆分归属——迁移仅改驻留位置，逻辑逐字节保持。
"""
import json
import random
import re
