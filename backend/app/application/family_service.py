...
from app.application.permission_service import _invalidate_admin_cache
...
async def redeem_invite_code(db: AsyncSession, user_id: int, code: str | None) -> dict:
    ...
    # 并发兜底：占用成功后、落关系前再校验一次上限（TOCTOU 防护，超限回滚无脏数据）
    if await count_sub_accounts(db, invite.creator_id) >= MAX_SUB_ACCOUNTS:
        raise HTTPException(status_code=409, detail="sub account limit reached")
