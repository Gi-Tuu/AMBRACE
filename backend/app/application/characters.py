"""AI 角色应用服务（F5-b，2026-08-31 自 api/characters.py 迁入）。

业务体：角色 CRUD 与归属校验/开场白 LLM 生成/内心世界聚合（任务+工具轨迹+检索轨迹+
运行笔记）/Lorebook Journal/世界事实/状态历史/情绪时间线。api/characters.py 只留
FastAPI 壳（收参→调本模块→返回）并对历史名字做门面重导出（F8 删旧时移除）。

不变量：删除角色=完全清除（级联清单与离开记忆语义）、system_prompt 随人格字段联动、
任务记录兜底口径（classify 归一）、lorebook 50 条上限——迁移仅改驻留位置，逻辑逐字节保持。
"""
...
_logger = get_logger("application.characters")
