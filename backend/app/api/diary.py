...
@router.get("/{character_id}/dates")
async def get_diary_dates(
    character_id: int,
    month: str = Query(..., description="月份，格式 YYYY-MM", pattern=r"^\d{4}-\d{2}$"),
    db: AsyncSession = Depends(get_db),
    user_id: int = Depends(get_current_user_id),
    lang: str = Header(default="zh"),
):
    """获取某月有日记的日期列表（轻量，只返回日期不返回正文）。"""
    await _check_character_owned(db, character_id, user_id, lang)
    prefix = f"{month}-"
    result = await db.execute(
        select(AIDiary.diary_date)
        .where(
            AIDiary.character_id == character_id,
            AIDiary.diary_date.like(f"{prefix}%"),
        )
        .order_by(AIDiary.diary_date)
    )
