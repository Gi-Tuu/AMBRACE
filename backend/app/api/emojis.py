...
async def list_emoji_packs(user_id: int = Depends(get_current_user_id)):
    """表情包列表（含已下载标记与表情内容；未下载包不返回表情明细）。已安装市场包从磁盘读 manifest 合并。"""
    packs = await emoji_service.list_packs(user_id)
    market = await emoji_market.list_installed_market_packs(user_id)
    return {"packs": packs + market}
...
async def list_market_emoji_packs(user_id: int = Depends(get_current_user_id)):
    """表情市场列表：远程索引包 + 已装标记。"""
    packs = await emoji_market.list_market_packs(user_id)
    return {"packs": packs}


@router.post("/market/packs/{pack_id}/download")
async def download_market_emoji_pack(
    pack_id: str, user_id: int = Depends(get_current_user_id), lang: str = Header(default="zh")
):
    """下载并安装市场表情包（sha256 + zip 安全校验 + 清单强校验）。"""
    return await emoji_market.download_market_pack(user_id, pack_id, lang)


@router.delete("/market/packs/{pack_id}")
async def uninstall_market_emoji_pack(
    pack_id: str, user_id: int = Depends(get_current_user_id), lang: str = Header(default="zh")
):
    """卸载市场表情包；内置包返回 400。"""
