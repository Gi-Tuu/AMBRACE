...
def _validate_index(data: bytes, cfg: dict) -> dict:
    ...
    for it in obj["items"]:
        ...
        items.append({
            ...
            "type": str(it.get("type", "http") or "http"),  # 48a：插件类型透传
            "icon": str(it.get("icon", "") or ""),  # 48a：图标透传
            "page": page,  # 48a：页面入口透传
            "has_page": bool(page),  # 48a：远程条目按 page 字段透传
...
def _scan_market_items() -> list[dict]:
    ...
    for path in registry._scan_dir(registry.EXAMPLE_DIR):
        ...
        items.append({
            ...
            "type": str(m.get("type", "http") or "http"),  # 48a：插件类型透传
            "icon": str(ex.get("icon", "") or m.get("icon", "") or ""),  # 48a：index extras / manifest 图标
            "page": page,  # 48a：页面入口透传
            "has_page": bool(page) and (path / page).is_file(),  # 48a：页面入口文件存在
...
def _merge_installed(items: list[dict]) -> list[dict]:
    ...
    for it in items:
        ...
        row["has_page"] = bool(pl.get("has_page")) if pl else bool(it.get("has_page"))  # 48a：已安装以实际为准
        out.append(row)
    return out
...
async def get_marketplace_item(name: str, user_id: int = Depends(get_current_user_id), lang: str = Header(default="zh")):
    ...
    row["enabled"] = bool(pl.get("enabled")) if pl else False
    row["installed_version"] = pl.get("version") if pl else None
    row["has_page"] = bool(pl.get("has_page")) if pl else bool(item.get("has_page"))  # 48a：已安装以实际为准
    return row
