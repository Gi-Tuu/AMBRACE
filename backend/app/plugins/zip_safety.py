...
from app.plugins.manifest import PAGE_EXT_WHITELIST  # noqa: F401  # 48a：页面扩展名白名单复用（页面托管端点引用）

# zip 安全检查常量（48a 对齐落雨：20MB / 60MB / 800 文件；路径穿越/符号链接/绝对路径校验保留）
MAX_ZIP_SIZE = 20 * 1024 * 1024  # 20MB
MAX_ZIP_ENTRIES = 800  # 解压炸弹防护（P1，2026-08-16）：条目数上限
MAX_ZIP_EXTRACT_SIZE = 60 * 1024 * 1024  # 解压总大小上限（P1）
