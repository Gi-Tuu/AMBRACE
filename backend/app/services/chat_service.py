...
async def _save_bio_update(character_id: int, bio_text: str, user_id: int):
    ...
    try:
        async with async_session_factory() as db:
            ...
            char = result.scalar_one_or_none()
            if char:
                char.self_statement = bio_text[:500]
                await db.flush()
                await db.commit()
...
async def send_and_receive(
    ...
    async with async_session_factory() as db:
        ...
        user_msg = ChatMessage(session_id=session_id, sender_type="user", content=content,
                               extra_meta=_q)
        db.add(user_msg)
        await db.flush()
    ...
    _trace_actions = _agent_actions.parse_actions(final_state.get("ai_response") or "")
    _trace_llm_calls = 1
    ...
    try:
        ...
        async def _save_browser_history(_char_id: int, _query: str) -> None:
            ...
            async with async_session_factory() as _db:
                ...
                if _ex is not None:
                    _ex.created_at = datetime.utcnow()
                else:
    ...
    clean_text, gen_prompt, img_text = _extract_gen_image(final_state.get("ai_response", ""))
    if gen_prompt:
        final_state["ai_response"] = clean_text
    ...
    try:
        ...
        _trace.enqueue_task_log(
            ...
            session_id=session_id,
            trigger="chat",
            route="search_loop" if _trace_searched else "direct",
    ...
    # Phase H：多工具轮次任务化（≥2 个动作 → agent_tasks 任务记录；只记不改行为，失败静默）
    try:
        from app.agent.task_engine import run_chat_task
        _all_steps = _agent_actions.actions_to_steps(_trace_actions) + _loop_steps
        if len(_all_steps) >= 2:
            asyncio.ensure_future(run_chat_task(
                character_id, user_id, session_id, content,
                _all_steps, final_state.get("ai_response") or "", True,
            ))
    except Exception:
        pass
    ...
    # ---- 2026-08-16 审计修复：HTTP /send 路径标记处理（与 chunked 对齐）----
    _ai_full = final_state.get("ai_response") or ""
    try:
        asyncio.ensure_future(_save_phone_desktop_notes(character_id, _ai_full))  # 日历/备忘落库用原始含标记文本
    except Exception:
        pass
    try:
        ...
        from app.scheduler.promise_service import create_event
        _timer_info = extract_timer(
            _ai_full, user_id=user_id, character_id=character_id,
            session_id=session_id, source_message_id=user_msg_id, sender="ai",
        )
        _cleaned = _strip_actions(_ai_full) or ""
        final_state["ai_response"] = strip_timer_tag(_cleaned) or ""
        if _timer_info:
            await create_event(_timer_info)
    except Exception as e:
    ...
    async with async_session_factory() as db:
        ai_msg = ChatMessage(
            session_id=session_id, sender_type="ai", content=final_state["ai_response"],
            extra_meta=_ai_meta,
        )
        db.add(ai_msg)
        await db.flush()
    ...
    # 认知循环 v2.1：反思结果落库（节点已计算，拿到 AI 消息 id 后记录；失败静默）
    try:
        from app.agent.reflection import persist_reflection
        if final_state.get("reflection_result"):
            await persist_reflection(character_id, user_id, ai_msg.id, final_state["reflection_result"])
    except Exception as e:
    ...
    asyncio.ensure_future(add_chat_memory_extraction(
        session_id, character_id, user_id, content, final_state["ai_response"],
        source_id=user_msg_id,
    ))
    ...
    try:
        ...
        asyncio.ensure_future(maybe_extract_topics(
            character_id, user_id, content, final_state["ai_response"],
    ...
    _trigger_state_eval(character_id, user_id, content, final_state["ai_response"], final_state.get("status_update"))

    # P5：记忆可靠度信号（确认/纠正，$0 规则）+ 异步事实核查（节流，失败静默）
    try:
        from app.memory.reliability import schedule_feedback_processing
        schedule_feedback_processing(character_id, user_id, content, final_state["ai_response"])
    except Exception:
        pass
    try:
        from app.memory.fact_check import schedule_fact_check
        schedule_fact_check(character_id, user_id, content, final_state["ai_response"])
    except Exception:
        pass
...
async def send_and_receive_chunked(
    ...
    """
    user_msg_id = None
    user_msg_info = None  # 回传用户消息正式 id（前端替换本地临时 id，保证删除可用；2026-08-15）
    ...
    initial_state = {
        "user_message": content, "character_id": character_id,
        "user_id": user_id, "session_id": session_id, "intent": "",
        "retrieved_memories": [], "context_messages": [], "character_info": {},
        "ai_response": "", "should_update_memory": False,
        "new_memories": [], "emotional_state": "",
        "bio_update": None, "status_update": None,
        "source_id": user_msg_id,
        "lang": lang,
        "reasoning_level": await _load_reasoning_level(character_id),
    ...
    _t0 = time.monotonic()
    final_state = await agent.ainvoke(initial_state)
    full_text = final_state["ai_response"]

    # Task Trace（Phase A，2026-08-16）：快照原始动作（标记未剥离前），流程结束统一落库（先只写不读）
    _trace_actions = _agent_actions.parse_actions(full_text)
    ...
    # 生图标记提取（聊天内AI发图）：清理标记后走原有拆分落库流程
    clean_text, gen_prompt, img_text = _extract_gen_image(full_text)
    if gen_prompt:
        full_text = clean_text

    # 日历/备忘录标记提取（Phase F 收口，2026-08-16）：保留提取结果供 extra_meta 前端小字展示；
    # 落库统一走 _save_phone_desktop_notes（→ execute_tool，去重/署名），不再内联重复逻辑
    _cal_note_text = None
    _memo_text = None
    try:
        _cal = _extract_cal_note(full_text)
        if _cal:
            _cal_note_text = _cal[1]
    except Exception:
        pass
    try:
        _memo_text = _extract_memo(full_text)
    except Exception:
        _memo_text = None
    try:
        asyncio.ensure_future(_save_phone_desktop_notes(character_id, full_text))
    except Exception:
        pass

    # 定时承诺解析：检测 [timer:xx] 或"洗n分钟澡"等时间承诺，创建定时事件
    try:
        from app.scheduler.promise_parser import extract_timer, strip_timer_tag
        from app.scheduler.promise_service import create_event
        ...
        _logger.warning("Timer parse failed: %s", e)

    # Task Trace（Phase A）：写 trace（chunked 路径，先只写不读；失败静默）
    try:
        from app.agent import trace as _trace
        _trace.enqueue_task_log(
            task_id=_trace.new_task_id(),
            ...
            trigger="chat",
            route="chunked",
            steps_json=json.dumps(_agent_actions.actions_to_steps(_trace_actions), ensure_ascii=False),
            llm_calls=1,
            tool_calls=len(_trace_actions),
            latency_ms=int((time.monotonic() - _t0) * 1000),
            status="ok",
        )
    except Exception as _te:
    ...
    # 认知循环 v2.1：反思结果落库（节点已计算，拿第一条 AI 块消息 id 记录；失败静默）
    try:
        from app.agent.reflection import persist_reflection
        if final_state.get("reflection_result") and saved_chunks:
            await persist_reflection(character_id, user_id, saved_chunks[0]["id"], final_state["reflection_result"])
    except Exception as e:
        _logger.warning("Reflection persist failed: %s", e)
    ...
    # 触发记忆提取
    asyncio.ensure_future(add_chat_memory_extraction(
        session_id, character_id, user_id, content, full_text,
        source_id=user_msg_id,
    ))
    ...
    try:
        from app.agent.topic_tracker import maybe_extract_topics
        asyncio.ensure_future(maybe_extract_topics(
            character_id, user_id, content, full_text,
            perception=final_state.get("perception"),
        ))
    except Exception:
        pass
    ...
    # 异步评估八维可视化状态（10 分钟节流，不阻塞回复）
    _trigger_state_eval(character_id, user_id, content, full_text, final_state.get("status_update"))

    # 生图：用户要求画图时异步生成并追加图片消息
    if gen_prompt:
        asyncio.ensure_future(_gen_image_flow(user_id, character_id, session_id, gen_prompt, img_text))
