...
async def _persist_ai_chunks(
    ...
    """把语义块按现有 chunked 的 extra_meta 规则落库（首块：reasoning/tools；末块：status/cal/memo）。
...
async def send_and_receive_stream(
    ...
    - user_message：用户消息落库回传（前端替换本地临时 id）；
    - delta：增量展示文本（打字机）；
    - block：完整语义块（已落库，含 id/created_at/extra_meta）；
    ...
    user_msg_id, user_msg_info = await _persist_user_message(
        session_id, user_id, character_id, content,
        quote=quote, save_user_message=True, shared_memory=True,
    )
    if user_msg_info:
