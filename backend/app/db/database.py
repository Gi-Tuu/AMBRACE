...
async def init_db():
    ...
    async with engine.begin() as conn:
        ...
        # #46 主账号管理（选择型）：users 表加 is_admin 列 + 一次性种子（幂等）
        # 仅在表中尚无任何 is_admin=1 时，从 settings.admin_user_ids（env）写入，避免覆盖 UI 管理结果
        if ucols and "is_admin" not in [c1[1] for c1 in ucols]:
            await conn.execute(sa_text("ALTER TABLE users ADD COLUMN is_admin BOOLEAN DEFAULT 0"))
            print("[migrate] users.is_admin added")
        _admin_count = (await conn.execute(sa_text("SELECT COUNT(*) FROM users WHERE is_admin = 1"))).scalar() or 0
        if _admin_count == 0 and settings.admin_user_ids:
            _seed_ids = [str(int(x)) for x in settings.admin_user_ids if str(x).strip()]
