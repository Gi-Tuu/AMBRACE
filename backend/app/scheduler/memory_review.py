...
async def run_memory_review(char_id: int, user_id: int, memory_id: int) -> bool:
    ...
    from app.scheduler.triggers import memory_review_enabled
    ...
    try:
        ...
        _review_reasoning = ""
        if _rl == 2:
            _resp = await chat_completion(messages=_msgs, temperature=0.85, max_tokens=256,
                                          include_reasoning=True, task="review", user_id=user_id)
            if isinstance(_resp, tuple):
                text, _review_reasoning = _resp
            else:
                text = _resp
        else:
            if _rl == 1:
