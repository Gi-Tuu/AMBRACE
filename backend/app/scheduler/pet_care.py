...
async def _pet_llm(messages: list[dict], char_id: int | None,
    ...
    """宠物通道 LLM 调用（2026-08-15）：按角色思考挡位开推理（1=提示词简短思考 / 2=深度思考）。

    返回 (content, reasoning)；挡位 0/1 时 reasoning 为空串。"""
    ...
    if level == 2:
        resp = await chat_completion(messages=messages, temperature=temperature, max_tokens=max_tokens, task="message",
                                     include_reasoning=True)
        if isinstance(resp, tuple):
            content, reasoning = resp
            if reasoning:
                _logger.info("Pet msg reasoning captured: %d chars (char=%s)", len(reasoning), char_id)
...
async def run_pet_remind(char_id: int, user_id: int, pet_id: int) -> bool:
    ...
    try:
        ...
        pet_fact = ""
        try:
            from app.services.pet_service import species_fact
    ...
    try:
        from app.services.pet_service import log_activity
