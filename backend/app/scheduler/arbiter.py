...
from app.scheduler.ai_social import collect_ai_social_events
from app.scheduler.group_active import collect_group_events
from app.scheduler.unfinished_topic import collect_unfinished_events, run_unfinished_topic
from app.scheduler.life_regression import collect_life_regression_events, run_life_regression
...
async def get_active_characters() -> list[dict]:
    ...
    from app.scheduler.triggers import get_active_characters as _get
    return await _get()
...
async def collect_timer_events() -> list[dict]:
    """到期定时承诺（最高优先级）"""
    from app.scheduler.promise_service import get_due_events
    events = await get_due_events()
    return [{"type": "timer", "priority": 4, "event": e} for e in events]


async def collect_special_events() -> list[dict]:
    """生日 / 节日 / 认识纪念日"""
    from app.scheduler.triggers import (
        get_birthday_candidates, get_holiday_candidates, get_anniversary_candidates,
    )
    items = []
    for c in await get_birthday_candidates():
        items.append({"type": "birthday", "priority": 3, "candidate": c})
    for c in await get_holiday_candidates():
        items.append({"type": "holiday", "priority": 3, "candidate": c})
    for c in await get_anniversary_candidates():
        items.append({"type": "anniversary", "priority": 3, "candidate": c})
    return items
...
async def collect_rhythm_events() -> list[dict]:
    ...
    from app.scheduler.triggers import get_daily_count
    items = []
    for char_info in await get_active_characters():
        try:
            char_id = char_info["character_id"]
            # 有 pending 计时器 → 跳过随机节律（AI 正在"洗澡/睡觉"）
            if await has_pending_timer(char_id):
                continue
            # 有未发完的剧情切片 → 跳过随机节律，避免剧情重叠
            if await has_pending_storyline(char_id):
                continue
            # 每日上限
...
async def collect_motivation_events() -> list[dict]:
    """情感渴望驱动的主动唤醒：渴望度 >= 阈值 → 主动搭话候选（priority=1）。
    ...
    try:
        for c in await get_active_characters():
            ...
            if not cid:
                continue
            score = await _compute_motivation(cid)
...
async def collect_plugin_events() -> list[dict]:
    """插件主动消息候选（proactive_candidate hook，优先级 1；日限额由插件内部维护）"""
    try:
    ...
    for r in results:
        ...
        if isinstance(cand, list):
            for c in cand:
                ...
                if not c.get("character_id") or not c.get("user_id"):
                    continue
                items.append({"type": "plugin", "priority": 1, "candidate": {**c, "plugin": r.get("plugin", "")}})
            continue
        ...
        if not cid or not uid:
            continue
        items.append({"type": "plugin", "priority": 1, "candidate": {**cand, "plugin": r.get("plugin", "")}})
    return items
...
async def run_tick() -> list[str]:
    ...
    # 1. 收集
    timer_events = await collect_timer_events()
    special_events = await collect_special_events()
    rhythm_events = await collect_rhythm_events()
    state_trigger_events = await collect_state_trigger_events()
    motivation_events = await collect_motivation_events()
    review_events = await collect_review_events()
    contextual_events = await collect_contextual_events()
    care_events = await collect_care_events()
    pet_events = await collect_pet_events()
    ai_care_events = await collect_ai_care_events()
    ai_adopt_events = await collect_ai_adopt_events()
    pet_visit_events = await collect_pet_visit_events()
    ai_social_events = await collect_ai_social_events()
    group_events = await collect_group_events()
    plugin_events = await collect_plugin_events()
    unfinished_events = await collect_unfinished_events()
    life_regression_events = await collect_life_regression_events()

    # 2. 合并并按角色分组（同角色保留全部事件，按优先级降序尝试）
    by_char: dict[int, list[dict]] = {}
    for item in (timer_events + special_events + rhythm_events + state_trigger_events + motivation_events
                 + review_events + contextual_events + care_events + pet_events + ai_care_events
                 + ai_adopt_events + pet_visit_events + ai_social_events + group_events + plugin_events
                 + unfinished_events + life_regression_events):
        char_id = None
        if item["type"] == "timer":
    ...
    for char_id, items in sorted(by_char.items()):
        ...
        items.sort(
            key=lambda it: (it["priority"], it.get("motivation", 0.0)), reverse=True
        )
        for item in items:
            _t0 = _time.monotonic()
            try:
...
async def _execute(item: dict) -> bool:
    ...
    if etype == "timer":
        ...
        try:
            ...
            if _rl == 2:
                _resp = await chat_completion(
                    messages=[{"role": "system", "content": "直接输出内容，不要加引号和标注。"},
                              {"role": "user", "content": hint}],
                    temperature=0.8, max_tokens=256,
                    include_reasoning=True, task="message",
                )
                if isinstance(_resp, tuple):
                    content, _reasoning = _resp
                    _timer_reasoning = _reasoning or ""
                    if _timer_reasoning:
                        _logger.info("Timer reasoning captured: %d chars (char=%d)", len(_timer_reasoning), char_id)
                else:
                    content = _resp
            else:
    ...
    if etype == "ai_social":
        from app.scheduler.ai_social import run_ai_social
    ...
    if etype == "group_active":
        from app.scheduler.group_active import run_group_active
    ...
    if etype == "pet_visit":
        from app.scheduler.pet_care import run_pet_visit
    ...
    if etype == "memory_review":
        from app.scheduler.memory_review import run_memory_review
    ...
    if etype == "pet_remind":
        from app.scheduler.pet_care import run_pet_remind
    ...
    if etype == "ai_care":
        from app.scheduler.pet_care import run_ai_care
    ...
    if etype == "ai_adopt":
        from app.scheduler.pet_care import run_ai_adopt
    ...
    if etype in ("greeting", "proactive_chat", "goodnight", "status_update", "motivation"):
        from app.scheduler.message_generator import generate_proactive_event
        ...
        from app.scheduler.moment_publisher import publish_moment
...
async def recover_on_startup() -> None:
    ...
    from app.scheduler.promise_service import recover_overdue_events
