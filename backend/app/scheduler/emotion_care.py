...
async def run_emotion_care(char_id: int, user_id: int, task_id: int) -> bool:
    ...
    try:
        ...
        _emotion_reasoning = ""
        if _rl == 2:
            _resp = await chat_completion(messages=_msgs, temperature=0.9, max_tokens=256,
                                          include_reasoning=True, task="emotion", user_id=user_id)
            if isinstance(_resp, tuple):
                text, _emotion_reasoning = _resp
            else:
                text = _resp
        else:
            if _rl == 1:
