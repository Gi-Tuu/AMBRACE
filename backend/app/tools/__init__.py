...
把 5 个内置工具（search / image_gen / note_calendar / note_memo / note_done）的**执行入口**注册到
...
设计约定：
- 内置工具的执行入口在 app/tools/builtin/*.py，execute 内惰性 import services（避免 import 循环）；
...
def register_builtin_tools() -> int:
    """注册全部内置工具（幂等：重复调用覆盖同名条目，无副作用）。返回登记数。
