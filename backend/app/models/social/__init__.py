# -*- coding: utf-8 -*-
"""社交域：平台档案/社交记忆（F6 聚合，2026-08-31；X5 后渠道自有模型归扩展包）。
...
X5（2026-09-01）：渠道自有五模型迁至对应渠道扩展包（main.py 加载期注册进
Base.metadata；表结构逐字节继承，存量数据零迁移）。
"""
...
# 延续 2026-08-09 渠道数据单向隔离铁律。
class PlatformProfile(Base):
    """平台表达档案（Module A）：显式管理不同平台的可见性/亲密度/记忆权限/语气。

    默认档位：app=private/full；外部渠道=public/limited/social/creative。
    platform 枚举预留 wechat/qq（v2 不实现）。
    """
    __tablename__ = "platform_profiles"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    platform: Mapped[str] = mapped_column(String(20), unique=True, index=True)  # app / 渠道名 / 预留 wechat/qq
...
class SocialMemory(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    platform: Mapped[str] = mapped_column(String(20), default="")                 # 渠道名 / 预留 wechat/qq
