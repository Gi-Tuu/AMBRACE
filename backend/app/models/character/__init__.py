...
class AICharacter(Base):
    ...
    # 角色绑定 LLM 配置（#68 P0-P2：默认/我的配置/主账号共享配置）。
    # ondelete=SET NULL（P3-2 硬化，2026-09-17）：删除 user_llm_configs 行时 DB 层自动清角色引用，
    # 与 llm_config_service.delete_config 的应用层显式置 NULL 同义（DB 兜底），迁移 e8f9a0b1c2d3 落库。
    user_llm_config_id: Mapped[int | None] = mapped_column(
        Integer, ForeignKey("user_llm_configs.id", ondelete="SET NULL"), nullable=True, default=None
    )
