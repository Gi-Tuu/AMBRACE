...
class GameEngine(ABC):
    ...
    async def persist_state(self, db) -> None:
        ...
        for p in self.players:
            ...
            row.is_spectator = bool(getattr(p, "is_spectator", False))
    ...
    def in_play_players(self) -> list:
        """当前仍在对局中的玩家：非观战且存活（区别于 active_players，后者不排除已出局者）。"""
        return [p for p in self.players if not p.is_spectator and p.alive]

    async def apply_surrender(self, seat: int) -> dict:
        """通用投降（确定性，零 LLM），所有游戏共用。

        返回：
          {"ok": False, "error": ...}                                 非法投降
          {"ok": True, "end": True,  "winner": "seat_x"/"draw",
           "events": [...]}                                           投降后应结束
          {"ok": True, "end": False, "events": [...]}                 多人 3+：转观战，对局继续

        约定：调用方负责 persist_event / settle / commit；本方法只改内存状态并产出事件。
        """
        p = self.player_at(seat)
        if p is None or p.is_spectator or not p.alive:
            return {"ok": False, "error": "该座次不是在场玩家，无法投降"}

        name = self.name_of(seat)
        phase = self.session.phase or "surrender"
        ...
        if len(remaining) <= 1:
            ...
            return {
                "ok": True, "end": True, "winner": winner,
                "events": [{
                    "event_type": "surrender", "actor_seat": seat, "phase": "result",
                    "content": f"{name}选择投降{tail}。",
                    "visibility": "public", "payload": {"by_seat": seat, "ended": True},
                }],
            }

        # 多人 3+：仅转入观战，对局继续
        return {
            "ok": True, "end": False, "winner": None,
            "events": [{
                "event_type": "surrender", "actor_seat": seat, "phase": phase,
                "content": f"{name}选择投降，转为观战，其余玩家继续。",
                "visibility": "public", "payload": {"by_seat": seat, "ended": False},
            }],
        }
