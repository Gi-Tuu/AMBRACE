...
def c_db(monkeypatch, tmp_path):
    ...
    async def _seed_parents():
        ...
        # （本文件账号口径：1；角色：1/2/3/4/6/7/9）
        from app.models.character import AICharacter
        from app.models.user import User
        async with factory() as db:
            db.add(User(id=1, username="supersede_u1", nickname="取代链用户"))
            for cid in (1, 2, 3, 4, 6, 7, 9):
                db.add(AICharacter(id=cid, user_id=1, name=f"取代链角色{cid}"))
            await db.commit()
