...
def ctx_db(monkeypatch, tmp_path):
    """临时库（模板库克隆，见 tests/_dbclone.py）：monkeypatch triggers 的 async_session_factory（不触碰 backend/data）"""
    ...
    async def _init():
        ...
        # 拆种子提交：克隆库默认开 FK（生产同款 PRAGMA），用例体插 chat_messages(session_id=1)
        # → 父行逐批落库 users → ai_characters → chat_sessions
        async with factory() as db:
            db.add(User(id=1, username="pc_u1", nickname="主人"))
            await db.commit()
        async with factory() as db:
