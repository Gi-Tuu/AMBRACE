...
def pi_db(monkeypatch, tmp_path):
    """临时库（模板库克隆，见 tests/_dbclone.py）+ 种子用户/角色：把 database / prospective_intent /
    extractor / facts 的工厂指向临时工厂。

    种子父行保留：prospective_intents / world_facts 本身无外键，但 ai_characters.user_id 有，
    故 users 先提交、角色后提交（避免同事务内的顺序依赖）。
    """
    ...
    async def _seed():
        ...
        async with factory() as db:
            ...
            # 此前夹具只种角色、用例却传 chat_session_id=7；护栏加上后暴露了这份失真。
