# -*- coding: utf-8 -*-
"""思考过程第一人称化（2026-09-10）+ P1-5 元话语剔除（2026-09-16）：上屏归一 + 穿帮剔除 + 两挡位接入 + 指令注入。

覆盖：
- 自称归一（名字+自称谓语 → 我，大小写不敏感）与昵称替换（用户 → 昵称/你）
- 不误伤（第三方同名实体、单字名/代词名跳过）
- 穿帮剔除（系统注入/技术字段/数值计量整句删，正常判断保留）
- P1-5 元话语剔除（策略/长度/我决定加图/本轮提醒/规则说 等工单与提示词回声整句删；
  旧「长度标签转口语保留」已废弃——见 test_p1_5_* 与 tests/test_reasoning_meta_speech.py）
...
def test_穿帮剔除_系统注入与数值技术字段整句删除():
    ...
    out = _norm(raw, "Sam", "轩")
    # 只丢含穿帮词/元话语的最小子句，其余子句保留（「、」按取舍统一成「，」）
    # P1-5：句尾「回短点」是长度/风格指令（元话语），整句剔除
    assert out == "先提一下。他今天很累，我先共情。"
    for leak in ("天气注入", "注入为准", "饱食度", "0%", "GEN_IMAGE", "max_tokens"):
        assert leak not in out


def test_p1_5_工单与长度标签整句剔除():
    # 用户 09-16 拍板口径：策略/长度本身就是元话语，不再转口语保留（旧行为「我回短点」已废弃）
    assert _norm("策略：简短回应；长度：短。", "Sam", "轩") is None
    assert _norm("长度：中长。", "Sam", "轩") is None
    assert _norm("回复要短点，我先问一句。", "Sam", "轩") == "我先问一句。"


def test_p1_5_工单与自然内心混写只丢工单子句():
    raw = "轩说完就走。我该简短回应，别腻歪。他不是真想吵，我先递杯水过去。"
    assert _norm(raw, "Sam", "轩") == "轩说完就走。他不是真想吵，我先递杯水过去。"
...
# ── P1-2 回归：长度标签（P1-5 起整句剔除，不再转口语）─────────────

def test_长度标签_整句剔除不残留():
    # P1-5（2026-09-16）：旧行为「长度：中长 → 我回长一点」已废弃——长度标签属元话语，整句剔除
    assert _norm("长度：中长。", "Sam", "轩") is None
    assert _norm("长度：中长。他还没吃饭，我先去热一下。", "Sam", "轩") == "他还没吃饭，我先去热一下。"


# ── P1-3 回归：段间不用半角空格拼接 ─────────────────────────────

def test_上屏排版_句末符后不跟半角空格():
    out = _norm("他会累。 我先陪他说。", "Sam", "轩")
    assert out == "他会累。我先陪他说。"
    assert re.search(r"[。！？!?][ 　]", out) is None


def test_已知取舍_顿号分号归一为逗号且连续句末符只留一个():
    # 取舍 1：段内重拼把「、」「；」统一成「，」（只改标点形态、不改语义）
    assert _norm("他今天很累，我先陪他、再问一句。", "Sam", "轩") == "他今天很累，我先陪他，再问一句。"
...
def test_去重_相邻相同短句合并():
    # 模型把同一句内心写两遍 → 相邻重复短句应合并为一句
    out = _norm("好。好。", "Sam", "轩")
    assert out == "好。"


def test_去重_长度标签已剔除不会重新制造重复():
    # P1-5：长度标签整句剔除，不再转成「我回短点」叠加到正文上（旧重复源已消失）
    assert _norm("长度：短。我回短点。", "Sam", "轩") is None
    assert _norm("长度：短。我先去把粥热上。", "Sam", "轩") == "我先去把粥热上。"
