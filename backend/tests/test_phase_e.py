...
def test_runtime_allow_tools_经execute_tool执行并再决策(monkeypatch):
    """allow_tools=True：动作经统一执行入口（note 工具复用 chat_service._execute_note_tool，
    内部仍走 tool_runner.execute_tool）→ observation 注入 → 再决策 1 次"""
    seen = {"char_ids": [], "states": [], "tools": []}
    _patch_runtime_build(monkeypatch, seen)
    # 首轮输出 MEMO 标记 → 工具执行 → 第二轮输出最终正文
    _patch_generate(monkeypatch, seen, ["[MEMO]喂猫[/MEMO]正文第一轮", "最终正文"])

    async def _fake_exec_note(tool_name, payload, char_id):
        seen["tools"].append((tool_name, payload, char_id))

    monkeypatch.setattr("app.services.chat_service._execute_note_tool", _fake_exec_note)
    _patch_session_resolve(monkeypatch)
    res = asyncio.run(runtime_mod.run_social_reply(
        character_id=11, user_id=4, session_id=99,
...
def test_runtime_allow_tools_未注册工具不执行不编造(monkeypatch):
    """未接执行入口的工具（如 search 需主链路回调）→ 不执行、剥离标记，不编造成功"""
    seen = {"states": [], "tools": []}
    _patch_runtime_build(monkeypatch, seen)
    _patch_generate(monkeypatch, seen, ["[SEARCH]查一下[/SEARCH]正文"])
    ...
    assert any(s.get("action") == "SEARCH" and s.get("ok") is False for s in res["steps"])
