...
class _FakeRes:
    """最小结果集替身：支持 scalar_one_or_none / scalars().all()。"""
...
class _FakeSession:
    ...
    async def execute(self, stmt, *a, **kw):
        ...
        - users 表查询（get_family_root_id / get_family_member_ids）→ 返回空（无家庭关系，
          账号自身即租户根）；
        - 其余（AICharacter 归属查询）→ 模拟 WHERE 归属谓词：仅当角色归属者 == 请求账号
...
def test_trigger_test_cross_tenant_denied(monkeypatch):
    """账号独立 P1：主账号触发别家角色（跨租户）→ 404，不得读到别家私聊历史。

    原实现只校验 is_admin，无角色归属校验（审计列为跨租户读）；现由 _check_char_owned 收敛。
    """
    _patch_api_deps(monkeypatch, (["不该被生成"], ""))
    foreign = _char()
    foreign.user_id = 999  # 别家账号
    client = TestClient(_make_app(foreign, ADMIN))
    r = client.post("/api/v1/proactive/trigger/test", json={"character_id": 1})
...
def test_alembic_migration_creates_new_tables(monkeypatch, tmp_path):
    ...
    assert "group_char_cognitions" in names  # #72 PR-C 逐角色认知（2026-09-15）
    assert "memory_write_receipts" in names  # #70 M3 写入回执（2026-09-15）
    assert "game_stats" in names  # #62 Phase 3 成就与统计（2026-09-15）
    assert "game_achievements" in names
    assert "game_content_overrides" in names
    # 不硬编码总数（每加一张表都要改死数字，已三次回归）：断言"必需表在" + 数量不回退
    assert len(names) >= 107  # 107 = 94 应用表 + 4 游戏表 + user_llm_configs + user_device_tokens + memory_archive + prospective_intents + user_facts + group_memories + domain_events
