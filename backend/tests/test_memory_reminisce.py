...
def r_db(monkeypatch, tmp_path):
    ...
    memory_review 的 async_session_factory（不触碰 backend/data）。"""
    ...
    async def _seed_parents():
        # _dbclone 默认开 FK（生产同款 PRAGMA）：memories / chat_sessions 的 user_id、
        # character_id 与 proactive_message_logs 的 character_id 需父行先存在
        # （本文件账号口径：3；角色：13/14/101-104/201）
        from app.models.character import AICharacter
        from app.models.user import User
        async with factory() as db:
            db.add(User(id=3, username="reminisce_u3", nickname="回忆用户"))
            for cid in (13, 14, 101, 102, 103, 104, 201):
                db.add(AICharacter(id=cid, user_id=3, name=f"回忆角色{cid}"))
            await db.commit()
