...
def mem_db(monkeypatch, tmp_path):
    """临时 SQLite 文件库（模板库克隆，见 tests/_dbclone.py）：monkeypatch 相关模块的
    ...
    async def _seed_parents():
        ...
        async with factory() as db:
            db.add(User(id=1, username="mem_chain_u1", nickname="记忆链用户"))
            db.add(AICharacter(id=1, user_id=1, name="记忆链角色"))
            await db.commit()
