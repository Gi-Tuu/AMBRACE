...
def mem_db(monkeypatch, tmp_path):
    ...
    context_builder 的 async_session_factory（不触碰 backend/data）"""
    tmp = str(tmp_path)
    ...
    async def _seed_parents():
        # _dbclone 默认开 FK（生产同款 PRAGMA）：daily_summaries.session_id 与
        # shared_events.user_id / character_id 需父行先存在（会话口径：7）
        from app.models.character import AICharacter
        from app.models.chat import ChatSession
        from app.models.user import User
        async with factory() as db:
            db.add(User(id=1, username="gen_u1", nickname="生成用户"))
            db.add(AICharacter(id=1, user_id=1, name="生成角色"))
            db.add(ChatSession(id=7, user_id=1, character_id=1))
            await db.commit()
