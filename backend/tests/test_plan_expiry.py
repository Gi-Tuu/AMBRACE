...
def expiry_db(monkeypatch, tmp_path):
    """临时库（模板库克隆，见 tests/_dbclone.py）+ 种子父行：monkeypatch
    maintain_plan_expiry / service / supersede 的 async_session_factory，
    并记录向量同步调用（不触碰 backend/data、不触真实 Chroma）。

    _dbclone 默认开 FK（生产同款 PRAGMA）：_seed 固定写 user_id=3/character_id=13，
    memories 上这两个是外键，故补齐父行（users 先提交、角色后提交）。
    """
    ...
    async def _seed_parents():
        async with factory() as db:
            db.add(User(id=3, username="pe_u3", nickname="用户3"))
            await db.commit()
...
def write_db(monkeypatch, tmp_path):
    """save_memory 路径：临时库（模板库克隆，见 tests/_dbclone.py）+ 种子父行 +
    向量/嵌入/后台任务全 noop（与 test_memory_chain.py 同法）。

    同 expiry_db：memories.user_id/character_id 是外键，save_memory 固定写 3/13，故补父行。
    """
    import app.memory.dedup as memdedup
    from app.models.character import AICharacter
    from app.models.user import User
